...
def Value.fst : Value (τ₁ :×: τ₂) → Value τ₁
  | VPair v₁ _ => v₁
def Value.snd : Value (τ₁ :×: τ₂) → Value τ₂
  | VPair _ v₂ => v₂
@[simp]
theorem AExpr.pair_fst : (VPair v₁ v₂).fst = v₁ := by unfold Value.fst; simp_all
@[simp]
theorem AExpr.pair_snd : (VPair v₁ v₂).snd = v₂ := by unfold Value.snd; simp_all
-- @[simp]
theorem AExpr.pair_shape (v: Value (τ₁ :×: τ₂)): v = VPair v.fst v.snd := by cases v; simp_all
...
def AExpr.value : AExpr [] τ → Value τ
  | AValue v => v
@[simp] theorem AExpr.value_value : (AValue v).value = v := by unfold AExpr.value; simp_all
...
instance fintype (τ: Ty) : Fintype (Value τ) :=
  ...
  have inj : ∀(τ₁ τ₂: Ty), Function.Injective (@f τ₁ τ₂) :=
    fun _ _ (a11, a12) (a21, a22) _ => by aesop;
