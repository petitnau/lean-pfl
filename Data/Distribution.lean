...
def Distribution (τ: Ty): Type :=
  Value τ -> Probability
notation "𝔻(" τ ")" => Distribution τ

namespace Distribution

theorem eq_ext : ∀(a b: 𝔻(τ)), a = b ↔ ∀v, a v = b v := by
...
instance instZero : Zero 𝔻(τ) := ⟨ fun _ => 0 ⟩
instance instOne : One 𝔻(τ) := ⟨ fun _ => 1 ⟩

------------------
-- INEQUALITIES --
------------------
instance instLE {τ: Ty}: LE 𝔻(τ) :=
  ⟨fun d1 d2 => ∀(v: Value τ), d1 v ≤ d2 v⟩

theorem le_ext : ∀(a b: 𝔻(τ)), a ≤ b ↔ ∀v, a v ≤ b v := by
  intro a b; apply Iff.intro; all_goals (intro h; assumption)

instance instLT {τ: Ty}: LT 𝔻(τ) :=
  ⟨fun d1 d2 => d1 ≤ d2 ∧ ∃(v: Value τ), d1 v < d2 v⟩

theorem lt_ext : ∀(a b: 𝔻(τ)), a < b ↔ a ≤ b ∧ ∃(v: Value τ), a v < b v := by
  intro a b; apply Iff.intro; all_goals (intro h; assumption)
...
instance instAdd (τ: Ty): Add (𝔻(τ)) := ⟨fun a b => λv => (a v) + (b v)⟩
theorem add_ext (a: 𝔻(τ)) (b: 𝔻(τ)) : a + b = (λ v => (a v) + (b v)) := rfl

@[simp]
theorem zero_add_p : ∀(a: 𝔻(τ)), 0 + a = a := by
  intro a; unfold HAdd.hAdd instHAdd Add.add instAdd
  unfold OfNat.ofNat Zero.toOfNat0 Zero.zero instZero; simp_all

@[simp]
theorem add_zero_p : ∀(a: 𝔻(τ)), a + 0 = a := by
  intro a; unfold HAdd.hAdd instHAdd Add.add instAdd
  unfold OfNat.ofNat Zero.toOfNat0 Zero.zero instZero; simp_all
...
instance instMul (τ: Ty): Mul (𝔻(τ)) := ⟨fun a b => λv => (a v) * (b v)⟩
theorem mul_ext (a: 𝔻(τ)) (b: 𝔻(τ)) : a * b = (λ v => (a v) * (b v)) := rfl

@[simp]
theorem zero_mul_p : ∀(a: 𝔻(τ)), 0 * a = 0 := by aesop

@[simp]
theorem one_mul_p : ∀(a: 𝔻(τ)), 1 * a = a := by
...
theorem pmul_ext (a: ℙ) (b: 𝔻(τ)) : a * b = (λ v => a * (b v)) := rfl

theorem pmul_le_pmul (a b: ℙ) (c d: 𝔻(τ)) : a ≤ b → c ≤ d → a * c ≤ b * d := by
  intro h1 h2; rw [le_ext]; intro v; rw [pmul_ext, pmul_ext];
...
instance instDiv (τ: Ty): Div (𝔻(τ)) := ⟨fun a b => λv => (a v) / (b v)⟩
theorem div_ext (a: 𝔻(τ)) (b: 𝔻(τ)) : a / b = (λ v => (a v) / (b v)) := rfl
...
noncomputable
instance instSub (τ: Ty): Sub (𝔻(τ)) := ⟨fun a b => λv => (a v) - (b v)⟩
...
noncomputable instance instSup {τ: Ty}: Sup 𝔻(τ) :=
  ⟨ fun d1 d2 v => (d1 v) ⊔ (d2 v) ⟩

noncomputable instance instInf {τ: Ty}: Inf 𝔻(τ) :=
  ⟨ fun d1 d2 v => (d1 v) ⊓ (d2 v) ⟩

noncomputable instance instSupSet {τ: Ty}: SupSet 𝔻(τ) :=
  ⟨ fun s => fun v => sSup {d v | d ∈ s} ⟩
theorem sSup_ext (D: Set 𝔻(τ)) : sSup D = (λv => sSup {d v | d ∈ D}) := by aesop

noncomputable instance instInfSet {τ: Ty}: InfSet 𝔻(τ) :=
  ⟨ fun s => fun v => sInf {d v | d ∈ s} ⟩
theorem sInf_ext (D: Set 𝔻(τ)) : sInf D = (λv => sInf {d v | d ∈ D}) := by aesop

instance bot {τ: Ty}: Bot 𝔻(τ) := ⟨0⟩
instance top {τ: Ty}: Top 𝔻(τ) := ⟨1⟩

lemma sSup_apply (T: Set (𝔻(τ))) : sSup {t i | t ∈ T} = (sSup T) i := by aesop

---------------
-- INSTANCES --
---------------

instance instCommMonoid (τ: Ty): CommMonoid (𝔻(τ)) :=
  { mul_assoc := by
...
instance instAddCommMonoid (τ: Ty): AddCommMonoid (𝔻(τ)) :=
  { add_assoc := by
...
instance instPartialOrder {τ: Ty}: PartialOrder 𝔻(τ) :=
...
noncomputable instance instCompleteLattice {τ: Ty}: CompleteLattice 𝔻(τ) :=
